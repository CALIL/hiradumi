...
class Hiradumi extends React.Component {
  ...
  setRowData() {
    ...
    this.factors.some((notValue, index) => {
      ...
      let height = this.state.size * this.factors[index]; // 一行に入る数
      ...
      if (scaleUpRatio < 2) {
        ...
        // 前の行の調整
        const prevRowData = this.props.data.slice(currentIndex - rowsData[rowsData.length - 1].length, currentIndex); // rowWidth分詰めたい
  ...
  render() {
    ...
    }, /*#__PURE__*/React.createElement("style", null, css), Array.from({
      length: this.state.rowsData.length
    }).map((notValue, index) => {
      return /*#__PURE__*/React.createElement(HiradumiRow, {

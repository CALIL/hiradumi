...
// 行の幅の範囲内にアイテムを入れる
const putItem = (currentItems, rowWidth, rowRatio, itemHeight, itemMargin) => {
    const items = [];
    let rowTotalWidth = 0;
    const height = itemHeight * rowRatio;
    currentItems.some((item) => {
        const hasAspect = item.properties && item.properties.aspect;
        const aspect = hasAspect ? item.properties.aspect : 0.666666;
...
const pushPreviousRow = (items, prevItems, rowTotalWidth) => {
...
const sortCenter = (items, sortLabel) => {
    if (sortLabel) {
...
const Hiradumi = (props) => {
    ...
    if (props.items === undefined || props.items === null || props.items.length === 0)
        return null;
    let Items = props.items.map(item => (Object.assign({}, item)));
    let Rows = [];
    let HiradumiDiv = null;
    ...
    return (React.createElement("div", { className: props.className ? props.className : 'hiradumi', ref: (element) => HiradumiDiv = element },
        React.createElement(List, { width: props.width, height: props.height, itemCount: Rows.length, itemSize: (index) => rowHeights[index], estimatedItemSize: estimatedItemSize, onScroll: (event) => {
                if (!HiradumiDiv)
                    return;
            ...
            if (typeof row.type === 'undefined') {
                ...
                return (React.createElement("div", { className: "row", style: rowStyle, key: 'row' }, row.map((item, index) => {
                    return (React.createElement("div", { key: 'item' + index, className: "item", id: item.id, style: {
                        ...
                        if (props.itemComponent) {
                            return React.createElement(props.itemComponent, { item: item, margin: props.itemMargin, sortKey: props.sortKey });
                        }
                        else {
                            return React.createElement(DefaultItem, { item: item, margin: props.itemMargin, sortKey: props.sortKey });
                        }
                    })()));
                })));
                // ヘッダー・フッター
                ...
                return (React.createElement("div", { className: "row", style: rowStyle },
                    React.createElement(row.component, null)));
            }
        })));
};
